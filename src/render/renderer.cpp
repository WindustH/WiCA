...
bool Renderer::initializeTTF()
{
    ...
    if (logger)
        logger->info("Start to initialize TTF.");
    ...
    {
        if (logger)
            logger->error("TTF initialization failed.");
        uiComponentsInitialized_ = false;
        return false;
    }
    uiComponentsInitialized_ = true;
    if (logger)
        logger->info("TTF initialized.");
    return true;
}
...
void Renderer::cleanupTTF()
{
    auto logger = Logger::getLogger(Logger::Module::Renderer);
    if (logger)
        logger->info("Start to clean up TTF.");
    if (uiComponentsInitialized_)
    {
        TTF_Quit();
        uiComponentsInitialized_ = false;
...
bool Renderer::loadFontInternal(const std::string &fontIdentifier, int fontSize, bool isFullPath)
{
    ...
    {
        if (logger)
            logger->error("TTF system not initialized. Cannot load font.");
        return false;
    }
    ...
    if (!uiFont_)
    {
        if (!isFullPath)
        {
            ...
            {
                if (logger)
                    logger->error("Failed for '" + pathToTry + "' (and as system font '" + fontIdentifier + "'). SDL_ttf Error: " + std::string(SDL_GetError()));
                fontLoadedSuccessfully_ = false;
                currentFontName_ = "";
                currentFontPath_ = "";
                return false;
        ...
        {
            if (logger)
                logger->error("Failed for full path '" + pathToTry + "'. SDL_ttf Error: " + std::string(SDL_GetError()));
            fontLoadedSuccessfully_ = false;
            currentFontName_ = "";
            currentFontPath_ = "";
            return false;
...
bool Renderer::loadDefaultFont(int fontSize)
{
    auto logger = Logger::getLogger(Logger::Module::Renderer);
    if (logger)
        logger->info("Start to load default font with size: " + std::to_string(fontSize));
    ...
    if (logger)
        logger->debug("Local asset default fonts failed. Trying system fallbacks.");
    ...
    for (int i = 0; systemFontFallbacks[i] != nullptr; ++i)
    {
        ...
        {
            if (logger)
                logger->warn("Loaded system fallback font '" + std::string(systemFontFallbacks[i]) + "' as default.");
            return true;
        }
    }

    if (logger)
        logger->error("Failed to load ANY default font (local or system). UI text will not be available.");
    return false;
}
...
void Renderer::reinitializeColors(const Rule &newConfig)
{
    ...
    {
        if (logger)
            logger->error("Provided newConfig is not loaded. Using fallback colors.");
    ...
    else
    {
        ...
        {
            if (logger)
                logger->debug("Default state " + std::to_string(configDefaultState) + " color not in map, adding it.");
...
bool Renderer::initialize(SDL_Window *window, const Rule &config)
{
    auto logger = Logger::getLogger(Logger::Module::Renderer);
    if (logger)
        logger->info("Start to initialize renderer.");
    if (!window)
    {
        if (logger)
            logger->error("Provided window is null.");
        return false;
    }
    sdlWindow_ = window;
...
for (const char* renderer_name : renderers)
{
    // 1. 创建属性列表
    ...
    // 3. 使用正确的函数签名创建渲染器
    sdlRenderer_ = SDL_CreateRendererWithProperties(props);
    ...
    {
        if (logger)
            logger->info("Successfully created hardware-accelerated renderer: " + std::string(renderer_name));
        break; // 成功创建，跳出循环
    }
    else
    {
        if (logger)
            logger->warn("Failed to create renderer '" + std::string(renderer_name) + "'. Trying next. Error: " + std::string(SDL_GetError()));
    }
}

// 如果所有硬件加速渲染器都失败了，则回退到软件渲染
if (!sdlRenderer_)
{
    if (logger)
        logger->warn("All hardware renderers failed. Trying SOFTWARE fallback.");
    ...
    if (!sdlRenderer_)
    {
        // 注意：这里的日志文本可以更正，因为它不是专门针对软件渲染器的函数
        if (logger)
            logger->error("SDL_CreateRendererWithProperties (SOFTWARE) also failed. Error: " + std::string(SDL_GetError()));
        return false;
    }
    else
    {
        if (logger)
            logger->info("Successfully created software renderer.");
    }
}
    ...
    {
        if (logger)
            logger->warn("Failed to initialize TTF system. UI text might not be available.");
    }
    else
    {
        ...
        {
            if (logger)
                logger->warn("Failed to load any default font during initialization.");
        }
    }
...
bool Renderer::setFontSize(int newSize)
{
    ...
    {
        if (logger)
            logger->error("Invalid font size: " + std::to_string(newSize));
        return false;
    }
    if (currentFontPath_.empty() && currentFontName_.empty())
    {
        if (logger)
            logger->warn("No font currently loaded (path and name are empty). Cannot change size. Trying to load default font with new size.");
    ...
    if (logger)
        logger->error("Failed to reload font '" + identifierToLoad + "' (and potentially '" + currentFontName_ + "') at new size " + std::to_string(newSize));
    return false;
}
...
bool Renderer::setFontPath(const std::string &fontPath, int fontSize)
{
    ...
    {
        if (logger)
            logger->error("Font path is empty.");
        return false;
    }
    if (fontSize <= 0)
    {
        if (logger)
            logger->error("Invalid font size: " + std::to_string(fontSize));
        return false;
    }
    ...
    if (logger)
        logger->error("Failed to load font from path/name: " + fontPath);
    return false;
}
...
void Renderer::setGridLineWidth(int width)
{
    ...
    if (width < 1)
    {
        gridLineWidth_ = 1;
        if (logger)
            logger->warn("Invalid grid line width " + std::to_string(width) + ". Setting to 1px.");
    }
    else
    {
...
void Renderer::renderCells(const CellSpace &cellSpace, const Viewport &viewport)
{
    ...
    else
    {
        ...
        if (cell_render_w == 0 && actual_cell_w_float > 0.0f)
            cell_render_w = 1;
        if (cell_render_h == 0 && actual_cell_h_float > 0.0f)
            cell_render_h = 1;

        if (cell_render_w <= 0 || cell_render_h <= 0)
            return; // Still no renderable area
    }
    ...
    {
        std::map<SDL_Color, std::vector<SDL_FRect>, SdlColorCompare> batchedRects;
        ...
        for (const auto &batch : batchedRects)
        {
            ...
            const std::vector<SDL_FRect> &rects = batch.second;
            if (!rects.empty())
            {
    ...
    {
        std::map<SDL_Color, std::vector<SDL_FPoint>, SdlColorCompare> batchedPoints;
        for (const auto &pInfo : pixelInfos)
        {
            SDL_FPoint newPoint;
            newPoint.x = pInfo.screenPos.x;
            newPoint.y = pInfo.screenPos.y;
            batchedPoints[pInfo.color].push_back(newPoint);
        }
        for (const auto &batch : batchedPoints)
        {
            const SDL_Color &color = batch.first;
            const std::vector<SDL_FPoint> &points = batch.second;
            if (!points.empty())
            {
                SDL_SetRenderDrawColor(sdlRenderer_, color.r, color.g, color.b, color.a);
                SDL_RenderPoints(sdlRenderer_, points.data(), static_cast<int>(points.size()));
            }
        }
    }
}
    ...
    if (drawGridLines && currentCellPixelSize > 0)
    {
        ...
        for (float wx = std::floor(worldTopLeftF.x); wx <= std::ceil(worldBottomRightF.x + 1.0f); ++wx)
        {
            ...
            if (screenPos.x + lineOffset >= -gridLineWidth_ && screenPos.x - lineOffset < screenW + gridLineWidth_)
            {
                if (gridLineWidth_ == 1)
                {
                    SDL_RenderLine(sdlRenderer_, screenPos.x, 0, screenPos.x, screenH);
                }
                else
                {
                    SDL_FRect lineRect = {screenPos.x - lineOffset, 0, gridLineWidth_, screenH};
        ...
        for (float wy = std::floor(worldTopLeftF.y); wy <= std::ceil(worldBottomRightF.y + 1.0f); ++wy)
        {
            ...
            if (screenPos.y + lineOffset >= -gridLineWidth_ && screenPos.y - lineOffset < screenH + gridLineWidth_)
            {
                if (gridLineWidth_ == 1)
                {
                    SDL_RenderLine(sdlRenderer_, 0, screenPos.y, screenW, screenPos.y);
                }
                else
                {
                    SDL_FRect lineRect = {0, screenPos.y - lineOffset, screenW, gridLineWidth_};
                    SDL_RenderFillRect(sdlRenderer_, &lineRect);
                }
            }
        }
...
void Renderer::renderGrid(const CellSpace &cellSpace, const Viewport &viewport)
{
    ...
    {
        if (logger)
            logger->warn("Default state " + std::to_string(defaultStateVal) + " color not found in map. Using fallback background.");
    }
...
void Renderer::renderMultiLineText(const std::string &text, int x, int y, SDL_Color color, int maxWidth, int &outHeight)
{
    ...
    std::string line;
    int currentY = y;
    int fontLineSkip = TTF_GetFontLineSkip(uiFont_);
    if (fontLineSkip <= 0)
        fontLineSkip = (uiFont_ && TTF_GetFontHeight(uiFont_) > 0) ? TTF_GetFontHeight(uiFont_) + 2 : currentFontSize_ + 2;

    while (std::getline(ss, line, '\n'))
    {
        SDL_Surface *surface = TTF_RenderText_Blended_Wrapped(uiFont_, line.empty() ? " " : line.c_str(), line.length(), color, static_cast<Uint32>(maxWidth));
        if (surface)
        {
            SDL_Texture *texture = SDL_CreateTextureFromSurface(sdlRenderer_, surface);
            if (texture)
            {
                SDL_FRect dstRect = {x, currentY, surface->w, surface->h};
                SDL_RenderTexture(sdlRenderer_, texture, nullptr, &dstRect);
            ...
            {
                if (logger)
                    logger->error("SDL_CreateTextureFromSurface failed for multi-line: " + std::string(SDL_GetError()));
                        ...
                        const Viewport &viewport)
{
    ...
    if (!isUiReady())
    {
        ...
        if (!uiErrorLoggedOnce)
        {
            ...
            {
                if (logger)
                    logger->error("UI Font not available (fontLoadedSuccessfully_ is false), cannot render text elements.");
            }
            else if (!uiFont_)
            {
                if (logger)
                    logger->error("UI Font pointer is null despite successful load flag. Cannot render text.");
            }
    ...
    if (!brushInfo.empty())
    {
        ...
        if (brushInfoMaxWidth < 100)
            brushInfoMaxWidth = 100;

        int actualBrushInfoTextWidth = 0;
        std::string firstLineBrush = brushInfo.substr(0, brushInfo.find('\n'));
        if (!firstLineBrush.empty())
        {
            TTF_MeasureString(uiFont_, firstLineBrush.c_str(), firstLineBrush.length(), 0, &actualBrushInfoTextWidth, nullptr);
        }
        else if (brushInfo.length() > 0)
        {
            TTF_MeasureString(uiFont_, brushInfo.c_str(), brushInfo.length(), 0, &actualBrushInfoTextWidth, nullptr);
        }
    ...
    if (!userMessage.empty())
    {
        ...
        if (messageRenderedHeight > 0)
        {
            ...
            {
                TTF_MeasureString(uiFont_, firstLineMsg.c_str(), firstLineMsg.length(), 0, &actualMsgTextWidth, nullptr);
            }
            else if (userMessage.length() > 0)
            {
                TTF_MeasureString(uiFont_, userMessage.c_str(), userMessage.length(), 0, &actualMsgTextWidth, nullptr);
            }

            SDL_FRect msgBgRect = {UIMargin, currentY,
                                   std::min(actualMsgTextWidth + 2 * textPadding, messageMaxWidth + 2 * textPadding),
                                   messageRenderedHeight + (2 * textPadding)};
            msgBgRect.w = std::min(msgBgRect.w, static_cast<float>(screenW - 2 * UIMargin));
    ...
    if (showCommandInput)
    {
        ...
        SDL_Surface *textSurface = TTF_RenderText_Blended_Wrapped(uiFont_, fullCommandText.c_str(), fullCommandText.length(), uiTextColor_, static_cast<Uint32>(cmdMaxWidth));
        if (!textSurface)
        {
            if (logger)
                logger->error("TTF_RenderText_Blended_Wrapped failed for command input: " + std::string(SDL_GetError()));
        }
        else
        {
            ...
            {
                if (logger)
                    logger->error("SDL_CreateTextureFromSurface failed for command input: " + std::string(SDL_GetError()));
            }
            else
            {
                ...
                SDL_FRect uiBackgroundRect = {UIMargin, cmd_y_pos, screenW - (2 * UIMargin), cmdBoxHeight};
                SDL_SetRenderDrawColor(sdlRenderer_, uiBackgroundColor_.r, uiBackgroundColor_.g, uiBackgroundColor_.b, uiBackgroundColor_.a);
                SDL_RenderFillRect(sdlRenderer_, &uiBackgroundRect);

                int text_y_offset = (cmdBoxHeight - cmdTextHeight) / 2;

                SDL_FRect uiTextRect = {UIMargin + textPadding, cmd_y_pos + textPadding + text_y_offset, cmdTextWidth, cmdTextHeight};
                uiTextRect.w = std::min(cmdTextWidth, cmdMaxWidth);

                SDL_RenderTexture(sdlRenderer_, textTexture, nullptr, &uiTextRect);
...
void Renderer::presentScreen()
{
    ...
    {
        if (logger)
            logger->error("sdlRenderer_ is null, cannot present.");
    }
}
...
void Renderer::cleanup()
{
    auto logger = Logger::getLogger(Logger::Module::Renderer);
    if (logger)
        logger->info("Renderer clean-up called.");
    if (uiFont_)
    {
        TTF_CloseFont(uiFont_);
        uiFont_ = nullptr;
